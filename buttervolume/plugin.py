...
TLS_CA_CERT_PATH = getconfig(config, "TLS_CA_CERT_PATH", None)
CLUSTER_HOST = getconfig(config, "CLUSTER_HOST", "0.0.0.0")
CLUSTER_PORT = int(getconfig(config, "CLUSTER_PORT", 8723))
...
def setup_routes(app):
    ...
    @app.route("/Plugin.Activate", ["POST"])
    @add_debug_log
    def plugin_activate(_):
        return {"Implements": ["VolumeDriver"]}

    @app.route("/VolumeDriver.Create", ["POST"])
    @add_debug_log
    @safe_handler
    ...
    @app.route("/VolumeDriver.List", ["POST"])
    @add_debug_log
    def volume_list(_):
        return list_volumes()
    ...
    @app.route("/VolumeDriver.Volume.Sync", ["POST"])
    @add_debug_log
    def volume_sync(req):
    ...
    @app.route("/VolumeDriver.Capabilities", ["POST"])
    @add_debug_log
    def driver_cap(_):
    ...
    @app.route("/VolumeDriver.Snapshot.Send", ["POST"])
    @add_debug_log
    ...
    @app.route("/VolumeDriver.Snapshot", ["POST"])
    @add_debug_log
    @safe_handler
    ...
    @app.route("/VolumeDriver.Snapshot.List", ["GET"])
    @add_debug_log
    def snapshot_list(_):
        snapshots = os.listdir(SNAPSHOTS_PATH)
        return {"Err": "", "Snapshots": snapshots}

    @app.route("/VolumeDriver.Snapshot.List/<name>", ["GET"])
    @add_debug_log
    @safe_handler
    ...
    @app.route("/VolumeDriver.Snapshot.Remove", ["POST"])
    @add_debug_log
    @safe_handler
    ...
    @app.route("/VolumeDriver.Schedule", ["POST"])
    @add_debug_log
    def schedule(req):
    ...
    @app.route("/VolumeDriver.Schedule.List", ["GET"])
    @add_debug_log
    def scheduled(_):
        """List scheduled jobs"""
        if os.path.exists(SCHEDULE_DISABLED):
    ...
    @app.route("/VolumeDriver.Schedule.Pause", ["POST"])
    @add_debug_log
    ...
    @app.route("/VolumeDriver.Schedule.Resume", ["POST"])
    @add_debug_log
    ...
    @app.route("/VolumeDriver.Snapshot.Restore", ["POST"])
    @add_debug_log
    @safe_handler
    ...
    @app.route("/VolumeDriver.Clone", ["POST"])
    @add_debug_log
    @safe_handler
    ...
    @app.route("/VolumeDriver.Snapshots.Purge", ["POST"])
    @add_debug_log
    @safe_handler
